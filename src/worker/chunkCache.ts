...
export class ChunkCache {
  ...
  enforceLimits(systemMemoryLimitBytes: number, gpuMemoryLimitBytes: number, protectedKeys = new Set<string>()): void {
    this.evictCpu(systemMemoryLimitBytes, protectedKeys);
    this.evictGpu(gpuMemoryLimitBytes, protectedKeys);
  }

  stats(pendingRequestCount: number, activeRequestCount: number): ResourceStats {
  ...
  private evictCpu(limit: number, protectedKeys: Set<string>): void {
    while (this.totalCpuBytes() > limit) {
      const candidate = this.oldest((entry) => entry.cpuBytes > 0 && !protectedKeys.has(entry.key));
      if (!candidate) break;
  ...
  private evictGpu(limit: number, protectedKeys: Set<string>): void {
    while (this.totalGpuBytes() > limit) {
      const candidate = this.oldest((entry) => entry.gpuBytes > 0 && !protectedKeys.has(entry.key));
      if (!candidate) break;
  ...
  private oldest(predicate: (entry: CacheEntry) => boolean): CacheEntry | undefined {
  ...
  private totalCpuBytes(): number {
    return [...this.entries.values()].reduce((sum, entry) => sum + entry.cpuBytes, 0);
  }

  private totalGpuBytes(): number {
    return [...this.entries.values()].reduce((sum, entry) => sum + entry.gpuBytes, 0);
  }
}